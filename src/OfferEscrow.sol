...
contract Escrow is ReentrancyGuard{
    ...
    function getOffer(uint256 offerId) external view returns (OffersStruct memory) {
        return offers[offerId];
    }

    //This returns all offers an owner has created. Anyone can call this an see his created offers.
    ...
    function applyOffer(uint256 offerId) external returns (bool) {
        ...
        require(msg.sender != ownerOf[offerId], "Owner cannot apply for his own Offer");
    ...
    function approveWorker(uint256 offerId, address worker) external returns (bool) {
        ...
        require(requested[worker][offerId], "Worker has not applied for the offer");
